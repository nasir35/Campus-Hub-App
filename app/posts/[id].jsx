...
const PostDetailCard = () => {
  ...
  if (!post) return null; // Ensure post is loaded before destructuring
  ...
  const handleUpdatePost = async () => {
    try {
      const response = await axios.put(`${env.API_URL}/posts/update/${id}`, {userId:author._id, content: editedContent });
  ...
  return (
    <SafeAreaView>
      ...
      <View className="bg-white p-4 my-2 rounded-2xl shadow-md">
        ...
        {editing ? (
          <TouchableOpacity onPress={handleUpdatePost} className="bg-blue-500 p-2 rounded-lg">
